$(document).ready(function() {
	...
	$("a.cart").hover(function() {
		$.ajax({
			url:'http://192.168.1.30:8080/new_store/client/ajax/getListCart',
			type: 'get',
			cache:false,
			dataType:'json',
			success: function(data, status) {
				...
				for (var i = 0; i < data.length; i++) {
					var path = "http://192.168.1.30:8080/new_store/public/admin/upload/images-book/" + data[i].options['img'];
					$('ul.cart-info').append('<li class="cart-item"><img class="img-cart" src="'+path +'"><p>'+data[i].name+'</p><p>'+data[i].qty+' x '+data[i].price+' VNĐ</p></li>');
				}
				$('ul.cart-info').append('<li class="cart-item"><a href="http://192.168.1.30:8080/new_store/client/purchase/cart" class="btn btn-primary">Xem giỏ hàng</a</li>')
	...
	$(document).on('click', 'button.watch-fast', function(event) {
		...
		$.ajax({
			url: 'http://192.168.1.30:8080/new_store/client/ajax/quickview',
			type: 'get',
			data: {
				id: id_book
			},
			dataType: 'json', 
			success: function(data, status) {
				$('div.quickview img').attr('src', 'http://192.168.1.30:8080/new_store/public/admin/upload/images-book/' + data.image);
	...
	$(document).on('click', 'a.cart-add', function(event) {
		...
		$.ajax({
			url: 'http://192.168.1.30:8080/new_store/client/ajax/addCartItem',
			type: 'GET',
			cache:false,
			data: {
				'id': id,
	...
	$('a.updateCart').click(function(event) {
		...
		$.ajax({
			url:'http://192.168.1.30:8080/new_store/client/ajax/update-Cart/'+row_id+'/'+quantity,
			type: 'get',
			cache: false,
	...
	$('a#btn-login').click(function(event) {
		...
		$.ajax({
			url: 'http://192.168.1.30:8080/new_store/client/ajax/login-ajax' ,
			type: 'GET',
			cache: false,
			dataType: 'json',
	...
	$('input[name=category]').change(function(event) {
		...
		$.ajax({
			url: 'http://192.168.1.30:8080/new_store/client/ajax/ajax-book' ,
			type: 'GET',
			cache: false,
			dataType: 'json',
			...
			success: function(data, status) {
				...
				for (var i = 0; i< data.length; i++) {
					...
					xhtml += '<img class="img-responsive" src="http://192.168.1.30:8080/new_store/public/admin/upload/images-book/' + data[i].image + '">';
					xhtml += '<div class="pro-detail"><h3 class="pro-name"><a href="http://192.168.1.30:8080/new_store/client/detail/' + data[i].id +'">'+data[i].name+'</a></h3>';
